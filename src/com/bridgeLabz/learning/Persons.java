...
public class Persons {
	...
	public void addContacts(int addressBook, String firstName, String lastName, String address, String city,
			String state, int zip, Long mobNum, String email) {
		contacts = new ArrayList<Details>();
		boolean result = contactMap.entrySet().stream().filter(p -> p.getValue().equals(firstName)).findAny()
				.orElse(null) != null;
		if (result) {
	...
	public void viewPersonByCity(String city) {
		cityMap.entrySet().stream().filter(x -> city.equals(x.getValue())).forEach(x -> {
	...
	public void viewPersonByState(String state) {
		stateMap.entrySet().stream().filter(x -> state.equals(x.getValue())).forEach(x -> {
	...
	public int countPersonsByCity(String city) {
		return (int) cityMap.entrySet().stream().filter(x -> city.equals(x.getValue())).count();
	}

	public int countPersonsByState(String state) {
		return (int) stateMap.entrySet().stream().filter(x -> state.equals(x.getValue())).count();
	}

	public void sortByName() {
		for (Map.Entry<List<Details>, Integer> entry : contactMap.entrySet()) {
			List<Details> listSortedByFirstName = entry.getKey().stream()
					.sorted(Comparator.comparing(Details::getFirstName)).collect(Collectors.toList());
			for (Details person : listSortedByFirstName) {
				System.out.println("Sorted by name: " + person.getFirstName() + " " + person.getLastName() + " "
						+ person.getAddress() + " " + person.getCity() + " " + person.getState() + " " + person.getZip()
						+ " " + person.getMobNum() + " " + person.getEmail());
			}
		}
	}
	...
	public void sortByCity() {
		for (Map.Entry<List<Details>, Integer> entry : contactMap.entrySet()) {
			List<Details> listSortedByCity = entry.getKey().stream().sorted(Comparator.comparing(Details::getCity))
					.collect(Collectors.toList());
			for (Details person : listSortedByCity) {
				System.out.println("Sorted by city: " + person.getFirstName() + " " + person.getLastName() + " "
						+ person.getAddress() + " " + person.getCity() + " " + person.getState() + " " + person.getZip()
						+ " " + person.getMobNum() + " " + person.getEmail());
			}
		}
	...
	public void sortByState() {
		for (Map.Entry<List<Details>, Integer> entry : contactMap.entrySet()) {
			List<Details> listSortedByState = entry.getKey().stream().sorted(Comparator.comparing(Details::getState))
					.collect(Collectors.toList());
			for (Details person : listSortedByState) {
				System.out.println("Sorted by state: " + person.getFirstName() + " " + person.getLastName() + " "
						+ person.getAddress() + " " + person.getCity() + " " + person.getState() + " " + person.getZip()
						+ " " + person.getMobNum() + " " + person.getEmail());
			}
		}
	...
	public void sortByZip() {
		for (Map.Entry<List<Details>, Integer> entry : contactMap.entrySet()) {
			List<Details> listSortedByZip = entry.getKey().stream().sorted(Comparator.comparing(Details::getZip))
					.collect(Collectors.toList());
			for (Details person : listSortedByZip) {
				System.out.println("Sorted by zip: " + person.getFirstName() + " " + person.getLastName() + " "
						+ person.getAddress() + " " + person.getCity() + " " + person.getState() + " " + person.getZip()
						+ " " + person.getMobNum() + " " + person.getEmail());
			}
		}
	...
	public void writeToFile() {
		new AddressBookFileIOService().writeData(contactMap);
	}
	...
	public void countEntries() {
		System.out.println("Number of entries are: " + new AddressBookFileIOService().countEntries());
	}

	List<Details> contactList = new AddressBookFileIOService().readFile();
